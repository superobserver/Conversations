...
def sieve_full_and_optimized_span(h=50, residue_k=17, start_y=1000, end_y=1999):
    epoch = 90 * (h * h) - 12 * h + 1
    a = 90
    b = -300
    c = 250 - epoch
    ...
    for x in range(1, int(new_limit) + 1):
        for l, m, z in operators:
            drLD_full(x, l, m, z, full_amp)
            full_ops += 1  # Count drLD calls as proxy
    ...
    for x in range(1, int(new_limit) + 1):
        for l, m, z in operators:
            ...
            opt_ops += marks  # Count effective marks
    ...
    print("Full operations (calls proxy):", full_ops)
    print("Optimized operations (marks):", opt_ops)

if __name__ == "__main__":
    sieve_full_and_optimized_span(h=50, start_y=1000, end_y=1999)  # Example span
    sieve_full_and_optimized_span(h=50, start_y=1000, end_y=1099)  # Small 100-wide
