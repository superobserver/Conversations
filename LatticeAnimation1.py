...
def create_animation(classes, base_start, num_maps, map_length, n_rows, m_cols, output_file='amplitude_time_series.gif', view_3d=False):
    if view_3d:
        fig = plt.figure(figsize=(10, 10))
        ax = fig.add_subplot(111, projection='3d')
        X, Y = np.meshgrid(range(m_cols), range(n_rows))
        surf = ax.plot_surface(X, Y, np.zeros((n_rows, m_cols)), cmap='viridis', vmin=0, vmax=1)
        ax.set_zlabel('Amplitude (Height)')
    else:
        fig, ax = plt.subplots(figsize=(10, 10))
        im = ax.imshow(np.zeros((n_rows, m_cols, 3)), interpolation='nearest')
    
    title_text = ax.set_title("Amplitude Map - Slice 0 (Start: 0, Temp: 0.0, Holes: 0)")
    ax.set_xlabel("Column")
    ax.set_ylabel("Row")
    ax.axis('off')
    
    temperatures = []  # Average amplitudes
    hole_counts = []   # Hole counts per slice
    
    def animate(frame):
        start = base_start + frame * map_length
        amplitudes = compute_amplitudes(classes, start, map_length)
        if view_3d:
            # 3D topography: height = normalized amplitude
            norm_amps = amplitudes / amplitudes.max() if amplitudes.max() > 0 else amplitudes
            norm_grid = norm_amps.reshape(n_rows, m_cols)
            ax.cla()  # Clear previous surface
            ax.plot_surface(X, Y, norm_grid, cmap='viridis', vmin=0, vmax=1)
            ax.set_title(title_text.get_text())  # Reuse title
            ax.set_zlim(0, 1)
        else:
        ...
        title_text.set_text(f"Amplitude Map - Slice {frame} (Start: {start}, Temp: {temp:.2f}, Holes: {hole_count})")
...
num_maps = 300   # Number of successive maps (slices) to generate
...
view_3d = False  # Set to True for 3D topographic view (peaks for high amp)
