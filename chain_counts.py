...
def count_chains(amplitude, max_k=6):
    ...
    for amp in amplitude:
        if amp == 0:
            ...
            current = 0
    return counts[1:]  # Exclude k=0

# Compute for sample silo over h=1 to 50
h_max = 50
sample_k = 17
chain_counts_per_epoch = {k: [] for k in range(1, 7)}
for h in range(1, h_max + 1):
    amp = compute_amplitude(h, sample_k)
    counts = count_chains(amp)
    for k in range(1, 7):
        chain_counts_per_epoch[k].append(counts[k-1])
    print(f"h={h}, Epoch={len(amp):,}, Chains (1-6): {counts}")

# Plot Chain Quantities
plt.figure(figsize=(12, 8))
for k in range(1, 7):
    plt.plot(range(1, h_max+1), chain_counts_per_epoch[k], label=f'{k}-chains')
plt.xlabel('h (Epoch)'), plt.ylabel('Number of Chains'), plt.title('Quantity of k-Chains per Epoch (Silo 17)')
