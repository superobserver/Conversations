...
def create_dual_view_animation(classes, num_epochs, show_traces_panel=True, output_file='dual_view_epoch_animation.gif'):
    ...
    red_overlay = ax_left.scatter([], [], color='red', alpha=0.4, s=25)  # visible baseline
    ...
    def animate(frame):
        ...
        # Update epoch boundary rectangle to show current active square
        epoch_side = math.ceil(math.sqrt(N_h))
        epoch_rect.set_width(epoch_side)
        epoch_rect.set_height(epoch_side)
        ...
        # Draw red entanglement overlay **only within the epoch square** (indices < N_h)
        marked = np.where(amplitudes > 0)[0]
        print(f"Frame {frame+1}: {len(marked)} marked positions detected")
        if len(marked) > 0:
            # Filter to ensure only valid epoch indices (redundant but explicit)
            ...
            if len(valid_marked) > 0:
                ...
                red_overlay.set_alpha(red_alpha.mean())  # average for whole scatter
            else:
...
if __name__ == '__main__':
    classes = [11]  # or [11,13]
    num_epochs = 40
    create_dual_view_animation(classes, num_epochs)