...
def create_dual_view_animation(classes, num_epochs, show_traces_panel=True, output_file='dual_view_epoch_animation.gif'):
    ...
    red_overlay = ax_left.scatter([], [], color='red', alpha=0.3, s=20)  # baseline alpha 0.3 + size 20
    
    trace_lines = []  # shared for both panels if needed
    ...
    def animate(frame):
        ...
        # Always draw red entanglement overlay on left (now guaranteed visible)
        marked = np.where(amplitudes > 0)[0]
        print(f"Frame {frame+1}: {len(marked)} marked positions detected")  # Debug confirmation
        if len(marked) > 0:
            ...
            # Alpha scales with entanglement strength, minimum 0.3
            red_alpha = 0.3 + 0.7 * (amps_marked / amps_marked.max())
            red_overlay.set_alpha(red_alpha.mean())  # average alpha for whole scatter
        else:
        ...
        # Draw traces (on right if enabled)
        ...
        artists = [im, red_overlay] + trace_lines
        return artists

    anim = animation.FuncAnimation(fig, animate, frames=num_epochs, interval=800, blit=False)  # blit=False for safety with scatter
