...
# New Chapter: Slice-Power Variation for Operator 7
...
# Graph 3: Operator 7 y Deflections
print("\nVisual Interlude 3: Operator 7 Deflections (saved as 'operator7_deflection.png')")
plt.figure(figsize=(10, 6))
for k in ALL_CLASSES:
    if y_per_silo[k]:
        plt.hist(y_per_silo[k], bins=20, alpha=0.5, label=f'Silo {k}')
plt.xlabel('y Values'), plt.ylabel('Frequency'), plt.legend(ncol=3), plt.title('y Deflections for Operator 7 Across Siloes')
plt.savefig('operator7_deflection.png')
print("Graph Description: Histograms show y spreads per silo; tight bounds around origin for min (49), max skew for extrema—variance as ratio of power.")
