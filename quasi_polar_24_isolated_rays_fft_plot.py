...
"""
Quasi-polar 3-D visualisation + FFT Power Spectra
h = 10 (epoch = 8881) → can scale to larger h

* 24 isolated operator rays
* 25th ray: aggregate sum wave
* FFT on each ray + aggregate
* Plots: 3D sieve + 24+1 FFT power spectra
"""
...
# 3.  FFT Analysis
# ----------------------------------------------------------------------
max_fft_n = min(limit, 100000)  # Safe for large h
n_top = 3
...
# ----------------------------------------------------------------------
# 6.  Plot 2: FFT Power Spectra (24 + 1)
# ----------------------------------------------------------------------
fig2, axes = plt.subplots(5, 5, figsize=(20, 16))
axes = axes.flatten()

for k in range(24):
    xf, power = spectra[k]
    axes[k].plot(xf, power, color='tab:blue', linewidth=1)
...
# Aggregate
axes[24].plot(xf_agg, power_agg, color='black', linewidth=1.5)
axes[24].set_title("Aggregate Sum Wave")
axes[24].set_xlabel("Frequency")
axes[24].set_ylabel("Power")
