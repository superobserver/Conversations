...
def create_dual_view_animation(
    ...
    show_red_entanglement=True,
    output_file='dual_view_epoch_animation.gif'
):
    ...
    # Red overlay only created if enabled
    red_overlay = None
    if show_red_entanglement:
        red_overlay = ax_left.scatter([], [], color='red', alpha=0.4, s=25)
    ...
    def animate(frame):
        ...
        # Update epoch boundary rectangle
        ...
        # Red entanglement overlay (only if enabled)
        if show_red_entanglement:
            marked = np.where(amplitudes > 0)[0]
            print(f"Frame {frame+1}: {len(marked)} marked positions detected")
            if len(marked) > 0:
                valid_marked = marked[marked < N_h]
                if len(valid_marked) > 0:
                    amps_marked = amplitudes[valid_marked]
                    row_marked = valid_marked // side
                    col_marked = valid_marked % side
                    red_overlay.set_offsets(np.column_stack((col_marked, row_marked)))
                    # Intensity: higher amplitude → stronger (darker) red
        ...
        artists = [im, epoch_rect] + trace_lines
        if show_red_entanglement and red_overlay is not None:
            artists.append(red_overlay)
        return artists
    ...
    print(f"Red entanglement overlay is {'ON' if show_red_entanglement else 'OFF'}")

# Run parameters
if __name__ == '__main__':
    classes = [11,13]  # or [11,13]
    num_epochs = 40
    show_traces_panel = True
    show_red_entanglement = False  # <--- New toggle flag
    
    create_dual_view_animation(
        classes,
        num_epochs,
        show_traces_panel=show_traces_panel,
        show_red_entanglement=show_red_entanglement
    )