...
# Segment parameters
n0 = 10000000  # large starting index (beyond early epochs)
w = 1000       # window width
...
# Group remnants per operator index
remnants_per_op = [[] for _ in operators]  # One list per op
...
for idx, (l, m, z) in enumerate(operators):
    ...
    for x in range(1, x_max + 1):
        ...
        if mark_pos <= n0 + w - 1:
            ...
            # Add relative index to remnants for this op
            rel_idx = mark_pos - n0
            remnants_per_op[idx].append(rel_idx)
    if interacts:
...
# Now remnants_per_op has length 24, one list per op
# Plot: Remnant Heatmap (Prints per Op)
data = np.zeros((len(operators), w))
for i, rem in enumerate(remnants_per_op):
    for pos in rem:
        if 0 <= pos < w:
            data[i, pos] = 1
...
plt.title('Remnant Prints (Marks) per Operator in Segment')
plt.xlabel('Segment Index'), plt.ylabel('Operator Index')
