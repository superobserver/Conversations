...
def create_combined_animation(classes, num_epochs, show_traces=True, output_file='amplitude_operator_traces.gif'):
    ...
    def animate(frame):
        ...
        if show_traces:
            # Draw operator traces colored by crystal face
        ...
        # Red entanglement overlay (always shown)
        ...
        title.set_text(f"Epoch {frame+1} | N={N_h} | Holes={np.sum(amplitudes == 0)}"
                       f"{' | Traces ON' if show_traces else ' | Traces OFF'}")
    ...
    print(f"Operator traces are {'enabled' if show_traces else 'disabled'}")

# Run parameters
if __name__ == '__main__':
    classes = [11]  # or [11,13] for twin overlay
    num_epochs = 30  # Adjust as needed
    show_traces = False  # <--- Toggle here (True = show wave fronts, False = pure amplitude)
    
    create_combined_animation(classes, num_epochs, show_traces=show_traces)